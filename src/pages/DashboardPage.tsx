...
// v14.1 - GOLD MASTER: Memoized, Accessible & SEO Controlled
...
import { HeadlessSEO } from '../components/HeadlessSEO'; // ✅ Adicionado para controle de título
import { useGamiPress } from '../hooks/useGamiPress';
import { useUserTracks } from '../hooks/useUserTracks';
import { useUserEvents } from '../hooks/useUserEvents';
import { useUserStreak } from '../hooks/useUserStreak';

// ... (Mantenha as interfaces SafeAchievement e ActivityItem iguais) ...
...
const DashboardPage: React.FC = () => {
  ...
  // ============================================================================
  // 1. DATA MEMOIZATION (Performance Boost 🚀)
  ...
  // Mock Activity (Memoized para não recriar ícones)
  const recentActivity = useMemo<ActivityItem[]>(() => [
    { icon: <Music className="text-primary" size={20} />, action: 'Downloaded', item: 'Zouk Nights Remix', xp: 10, time: '2 hours ago' },
    { icon: <Heart className="text-accent" size={20} />, action: 'Favorited', item: 'Electric Dreams', xp: 5, time: '5 hours ago' },
    { icon: <Calendar className="text-success" size={20} />, action: 'RSVP\'d to', item: 'Summer Vibes Festival', xp: 25, time: '1 day ago' },
  ], []);

  // Animation Variants
  const containerVariants = { visible: { opacity: 1, transition: { staggerChildren: 0.1 } }, hidden: { opacity: 0 } };
  ...
  return (
    <>
      {/* ✅ SEO: Define Título da Aba + NOINDEX (Área Privada) */}
      <HeadlessSEO 
        title={`Dashboard - ${user.name} | DJ Zen Eyer`}
