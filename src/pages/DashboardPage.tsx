...
const DashboardPage: React.FC = () => {
  ...
  // 🎮 Hook com dados reais do GamiPress
  ...
  // ✅ User stats calculados de forma eficiente
  const userStats = useMemo(() => {
    ...
    return {
      ...
      progressPercentage,
      totalTracks: 0, // TODO: Implementar quando necessário
      eventsAttended: 0, // TODO: Implementar quando necessário
      streakDays: gamipress.streak?.current || 0,
      tribeFriends: 12 // TODO: Implementar friends quando necessário
    };
  }, [gamipress.points, gamipress.streak]);

  // ✅ Recent Activity com dados REAIS do GamiPress
  const recentActivity = useMemo(() => {
    const safeAchievements = (gamipress.achievements && Array.isArray(gamipress.achievements)) 
      ? gamipress.achievements 
      : [];

    if (safeAchievements.length > 0) {
      return safeAchievements.slice(0, 4).map(ach => ({
        icon: <Trophy className="text-primary" size={20} />,
        action: 'Unlocked',
        item: ach.title || 'Achievement',
        xp: ach.points || 10,
        time: ach.earnedDate 
  ...
  const achievements = useMemo(() => {
    const allAchievements = (gamipress.allAchievements && Array.isArray(gamipress.allAchievements)) 
      ? gamipress.allAchievements 
      : [];

    if (allAchievements.length > 0) {
      return allAchievements.slice(0, 6).map(ach => ({
        emoji: ach.image ? '' : '🏆',
        image: ach.image || '',
        title: ach.title || 'Achievement',
        description: ach.description || ach.excerpt || 'Achievement unlocked!',
              ...
              <div className="flex-1 min-w-[300px]">
                ...
                <div className="space-y-2">
                  ...
                  <div className="h-3 bg-background rounded-full overflow-hidden">
                    <motion.div 
                      initial={{ width: 0 }}
                      animate={{ width: `${userStats.progressPercentage}%` }}
