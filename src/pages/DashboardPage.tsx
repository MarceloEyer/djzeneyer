    ...
    <div className="pt-24 pb-16 min-h-screen">
      <div className="container mx-auto px-4">
            ...
            <div className="flex items-center gap-6 flex-wrap">
              ...
              <div className="flex-1 min-w-[300px]">
                ...
                <p className="text-white/70 text-lg mb-4 flex items-center gap-2">
                  ...
                  {userStats.rank} • {t('gamification.level')} {userStats.level}
                </p>
                <div className="space-y-2" role="progressbar" aria-valuenow={userStats.progress} aria-valuemin={0} aria-valuemax={100}>
                  <div className="flex justify-between text-sm">
                    <span className="text-white/80 flex items-center gap-1"><TrendingUp size={16} /> {t('dashboard.progressToLevel', { level: userStats.level + 1 })}</span>
              ...
              >
                <Zap size={20} /> {t('dashboard.boostXP')}
              </motion.button>
            </div>
          </div>
        </motion.div>
        ...
        <motion.div variants={containerVariants} initial="hidden" animate="visible" className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6 mb-12">
          {[
            { icon: Music, value: userStats.totalTracks, label: t('dashboard.tracksDownloaded'), color: 'text-primary', loading: false },
            { icon: Calendar, value: userStats.eventsAttended, label: t('dashboard.eventsAttended'), color: 'text-success', loading: false },
            { icon: Target, value: userStats.streakDays, label: userStats.streakFire ? t('dashboard.dayStreakFire') : t('dashboard.dayStreak'), color: 'text-warning', loading: gamipress.loading },
            { icon: Users, value: userStats.tribeFriends, label: t('dashboard.tribeFriends'), color: 'text-accent', loading: false },
          ].map((stat, i) => (
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.3 }} className="lg:col-span-2">
            <div className="card p-6 h-full min-h-[400px]">
              <div className="flex items-center justify-between mb-6">
                ...
                <button onClick={() => navigate('/my-account')} className="text-primary hover:underline text-sm transition-all hover:scale-105">{t('gamification.viewAll')} →</button>
              </div>
              ...
              <div className="space-y-4">
                {gamipress.loading ? (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="text-sm text-white/50 flex items-center gap-1 mt-1">
                            <Clock size={12} /> {getTimeAgo(act.timestamp, t)}
                          </div>
                        </div>
                  ...
                  <div className="text-center py-12">
                    ...
                    <p className="text-white/50">{t('dashboard.noActivity')}</p>
                  </div>
                )}
              </div>
            </div>
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.3 }} className="space-y-6">
            ...
            <div className="card p-6">
              <h3 className="text-xl font-bold font-display mb-4 flex items-center gap-2"><Zap className="text-primary" size={20} /> {t('dashboard.quickActions')}</h3>
              <div className="space-y-3">
                {[
                  { icon: Music, label: t('dashboard.browseMusic'), path: '/music', variant: 'primary' },
                  { icon: Calendar, label: t('dashboard.viewEvents'), path: '/events', variant: 'outline' },
                  { icon: Gift, label: t('dashboard.visitShop'), path: '/shop', variant: 'outline' },
                ].map((action, i) => (
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.5 }} className="mt-12">
          <div className="card p-6 md:p-8">
            <div className="flex items-center justify-between mb-8 flex-wrap gap-4">
              <h2 className="text-xl md:text-2xl font-bold font-display flex items-center gap-2"><Award className="text-primary" size={28} /> {t('dashboard.yourAchievements')}</h2>
              <div className="text-white/70 flex items-center gap-2"><Trophy className="text-warning" size={20} /> <span className="text-primary font-bold text-xl">{unlockedCount}</span> <span>{t('dashboard.unlocked')}</span></div>
            </div>
            ...
            {safeAchievements.length > 0 ? (
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                {safeAchievements.map((achievement, i) => (
                  <motion.div key={achievement.id || i} whileHover={{ scale: 1.05 }} className="bg-surface/50 rounded-lg p-4 text-center transition-all border border-primary/30 hover:bg-surface/70 hover:border-primary cursor-pointer shadow-lg hover:shadow-primary/20">
                    ...
                    <div className="mt-2 text-xs text-success flex items-center justify-center gap-1 font-bold">{t('dashboard.unlocked')} <Award size={10} /></div>
                  </motion.div>
                ))}
              </div>
            ) : (
              ...
              <div className="text-center py-12 border border-dashed border-white/10 rounded-xl bg-white/5">
                ...
                <h3 className="text-xl font-bold text-white mb-2">{t('dashboard.startJourney')}</h3>
                <p className="text-white/60 mb-6 max-w-md mx-auto">{t('dashboard.startJourneyDesc')}</p>
                <button onClick={() => navigate('/music')} className="btn btn-primary">{t('dashboard.startListening')}</button>
              </div>
            )}
          </div>
        </motion.div>
