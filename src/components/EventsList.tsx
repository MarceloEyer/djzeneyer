...
const getCompleteEventData = (event: BandsintownEvent) => {
    ...
    // Fallback de 4 horas se não houver data final

    const endDate = new Date(eventDate.getTime() + (4 * 60 * 60 * 1000)); 

    

    // GARANTIA DE DADOS (O Fix do Google Search Console)
    ...
    return {
      ...
      // Fallback de Descrição (Essencial para o Warning Amarelo)
      ...
      // Fallback de Preço (Para o erro de 'offers')

      price: '0',

      priceCurrency: (country === 'Brazil' || country === 'BR') ? 'BRL' : 'USD'
