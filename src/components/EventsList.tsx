...
function EventsListInner({ limit = 10, showTitle = true, variant = 'full' }: EventsListProps) {
  ...
  const skeletonElements = useMemo(() => {
    return Array.from({ length: limit }).map((_, i) => {
      ...
      return (
        <div
          key={`skeleton-${i}`}
          className={`animate-pulse bg-surface/30 border border-border/5 rounded-xl overflow-hidden ${skeletonHeight}`}
        >
    ...
    return (
      <div className="w-full">
        {variant === 'full' && showTitle && (
          <div className="h-9 w-48 bg-text/5 animate-pulse rounded-lg mx-auto mb-8" />
        )}
  ...
  if (processedEvents.length === 0) {
    return (
      <div className="text-center py-12 text-text/60">
        <Calendar size={48} className="mx-auto mb-4 text-text/20" aria-hidden="true" />
  ...
  return (
    <div className="w-full">
      {variant === 'full' && showTitle && (
        <h2 className="text-3xl font-bold mb-8 text-center font-display text-text">
          {t('events_title')}
        </h2>
      )}
        ...
        {processedEvents.map((event) => {
          ...
          if (variant === 'compact') {
            return (
              <article
                key={event.event_id}
                className="card hover:border-primary/50 transition-all duration-300 group bg-surface/30 border border-border/5 rounded-xl overflow-hidden"
              >
                <Link to={detailHref} className="flex items-start gap-4 p-4">
                  <time dateTime={event.starts_at} className="flex-shrink-0 text-center bg-surface rounded-lg p-3 border border-border/10 min-w-[70px]">
                    <div className="text-2xl font-bold text-primary">{event.day}</div>
                    <div className="text-xs uppercase text-text/60">{event.month}</div>
                  </time>
                  <div className="flex-1 min-w-0">
                    <h3 className="font-bold text-text mb-1 line-clamp-1 group-hover:text-primary transition-colors text-left" dangerouslySetInnerHTML={{ __html: event.sanitizedTitle }} />
                    <div className="space-y-1 text-sm text-text/70 text-left">
          ...
          return (
            <article
              key={event.event_id}
              className="card group hover:border-primary/50 transition-all duration-300 overflow-hidden bg-surface/30 border border-border/5 rounded-2xl"
            >
              <Link to={detailHref}>
                <div className="relative aspect-[16/9] overflow-hidden">
                  <div className="w-full h-full bg-gradient-to-br from-surface to-background flex items-center justify-center relative p-6 text-center group-hover:scale-105 transition-transform duration-500">
                    ...
                    <div className="relative z-10 w-full">
                      <h3
                        className="text-2xl font-black uppercase tracking-tighter font-display text-text/90 leading-[0.9] break-words line-clamp-3"
                  ...
                  <div className="absolute top-4 left-4">
                    <div className="bg-background/80 backdrop-blur-md border border-border/10 rounded-lg p-2 text-center min-w-[60px]">
                      <div className="text-xl font-bold text-primary leading-none">{event.day}</div>
                      <div className="text-[10px] uppercase font-semibold text-text/70 mt-1">
                        {event.month}
                      </div>
                    </div>
                  </div>
                ...
                <div className="p-6">
                  ...
                  <div className="space-y-3 text-sm text-text/60">
