...
export const UserProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const fetchUserDetails = async (token: string, email: string) => {
    try {
      ...
      let gamipressAchievements: GamiPressEarning[] = []; 
      let gamipressRanks: GamiPressEarning[] = [];       
      ...
      try {
        // Fetch all GamiPress user earnings for the user
        const allEarningsResponse = await fetch(`${window.wpData.restUrl}wp/v2/gamipress-user-earnings?user_id=${userData.id}&per_page=100&context=view`, { 
        ...
        if (allEarningsResponse.ok) {
          ...
          gamipressPoints = allEarnings.filter(e => e.post_type === 'points_award').reduce((acc, curr) => {
            acc[curr.points_type || 'points'] = (acc[curr.points_type || 'points'] || 0) + (curr.points || 0);
            return acc;
          }, {});

          gamipressAchievements = allEarnings.filter(e => e.post_type === 'achievement' && e.status === 'publish');
          gamipressRanks = allEarnings.filter(e => e.post_type === 'rank' && e.status === 'publish');
          
          gamipressXP = gamipressPoints.points || 0; 
          if (gamipressRanks.length > 0) {
            const highestRank = gamipressRanks.sort((a,b) => b.id - a.id)[0]; 
            gamipressRankName = highestRank.title.rendered;
            gamipressLevel = gamipressRanks.length; 
          }
        } else {
          console.warn("[UserContext] Failed to fetch GamiPress earnings:", allEarningsResponse.status);
        }
        console.log("[UserContext] GamiPress data fetched:", { gamipressPoints, gamipressAchievements, gamipressRanks });

      } catch (gamipressErr) {
        console.warn("[UserContext] Failed to fetch GamiPress data (catch block):", gamipressErr);
      }
