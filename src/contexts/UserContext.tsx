...
export const UserProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => { if (config.siteUrl) setSdk(new SimpleJwtLogin(config.siteUrl)); }, [config.siteUrl]);
  ...
  const loginWithGoogle = () => {
    ...
    try {
      ...
      const params = new URLSearchParams({
        ...
        scope: 'openid profile email', access_type: 'offline', prompt: 'consent', state: state
      });
