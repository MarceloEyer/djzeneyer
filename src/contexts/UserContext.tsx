...
interface DecodedJwt {
  data: {
    user: {
      ...
      user_email: string;
      display_name: string;
      roles?: string[]; // Esperamos receber as roles do token
    }
  };
...
export const UserProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const setUserFromToken = (token: string) => {
    try {
      ...
      const loggedInUser: WordPressUser = {
        ...
        isLoggedIn: true,
        token: token,
        // CORRIGIDO: Lendo as roles do token, com 'subscriber' como padrão
  ...
  const register = async (name: string, email: string, password: string) => {
    ...
    setError(null);
    try {
      // CORRIGIDO: Usando o endpoint /register em vez de /users
  ...
  const loginWithGoogle = () => {
    ...
    try {
      ...
      // CORRIGIDO: Voltamos para a URL ?rest_route= que é mais robusta e adicionamos o state
