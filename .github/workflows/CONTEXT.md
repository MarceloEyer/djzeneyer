...
4. **Hashed Asset Retention:** Antes de promover `dist-next`, preserve `dist/assets` antigos com `rsync --ignore-existing`. Abas abertas e HTML cacheado podem pedir chunks lazy da build anterior; apagar esses arquivos causa `ChunkLoadError` ate o usuario atualizar. Depois da promocao atomica, remova assets com mais de 30 dias para evitar crescimento ilimitado em disco.
