...
4. **Hashed Asset Retention:** Antes de promover `dist-next`, o deploy tenta preservar `dist/assets` antigos com `rsync --ignore-existing` em modo best-effort. Abas abertas e HTML cacheado podem pedir chunks lazy da build anterior; se a preservação falhar, o deploy continua com warning e esses usuários podem precisar atualizar a página. Depois da promoção atômica, assets com mais de 30 dias são removidos para evitar crescimento ilimitado em disco.
