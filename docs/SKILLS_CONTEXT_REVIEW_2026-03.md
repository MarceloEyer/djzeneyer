...
## Memória operacional para próximas tarefas 🧠

Para garantir continuidade entre tarefas, este review deve ser usado junto com `docs/AI_LEARNINGS_LOG.md` como base de contexto persistida em repositório.

### O que já foi aprendido e deve ser reaproveitado
- Regras críticas de i18n, SEO e centralização de queries são os maiores fatores de consistência.
- A maior fonte de divergência entre bots está na sobreposição de documentos de instrução sem prioridade explícita.
- PRs ficam mais auditáveis quando incluem avaliação explícita de sugestões de outros bots.

### Impacto estimado na próxima sprint
| Indicador | Antes | Depois (estimado) |
|---|---:|---:|
| Retrabalho por conflito de instruções | Alto | Médio/Baixo (-40% a -60%) |
| Tempo de alinhamento no início da task | 100% baseline | 70% a 80% (-20% a -30%) |
| Clareza de decisão técnica no PR | Média | Alta (+30% a +50%) |
...
## Revalidação do que já foi implementado (estado atual) 🔎

### Evidências verificadas no código
- `useQueries.ts` segue como fonte central de queries e mantém uso de `_fields` em múltiplos endpoints.
- `HeadlessSEO` segue ativo e amplamente usado nas páginas.
- `usePrefetchOnHover` está integrado na Navbar para navegação percebida mais rápida.
- Rotas seguem arquitetura com lazy loading em `AppRoutes.tsx`.

### Avaliação de aderência (hoje)

| Item proposto no PR anterior | Situação atual | Veredito |
|---|---|---|
| Memória operacional (`AI_LEARNINGS_LOG`) | Implementado e útil para onboarding de agentes | ✅ Manter |
| Review estruturado de contexto/skills | Implementado; ainda relevante | ✅ Manter |
| Criar `AI_GOVERNANCE.md` (P0) | Implementado (`docs/AI_GOVERNANCE.md`) | ✅ Concluído |
| Checklist por tipo de tarefa | Parcial (conceitual, não operacional) | ⚠️ Evoluir |
| Budgets de performance | Ainda sem números canônicos no repositório | ⚠️ Definir baseline |

### Estimativa de impacto se executar os itens pendentes

| Item pendente | Ganho técnico estimado | Ganho operacional estimado |
|---|---:|---:|
| Monitorar aderência ao `AI_GOVERNANCE.md` canônico | - | 20–40% mais consistência entre agentes |
| Checklist operacional por task type | - | 30–50% menos retrabalho em revisão |
| Budgets de payload/bundle/TTFB | 10–25% melhor tempo percebido | 20–35% mais previsibilidade de entrega |

### Recomendação final
As alterações anteriores **continuam boas e válidas** para o projeto no estado atual. Como `AI_GOVERNANCE.md` já foi implementado, o foco agora é **sustentação**: monitorar aderência, atualizar budgets com métricas reais e evitar drift entre documentos.


## Plano de mitigação de conflitos de merge 🛡️

| Abordagem | Risco de conflito | Observação |
|---|---:|---|
| Editar `AGENTS.md`/`GEMINI.md` em toda entrega | Alto | Arquivos globais mudam com frequência |
| Centralizar em `docs/AI_GOVERNANCE.md` | Baixo/Médio | Menos superfície de conflito |
| Usar snippet de PR em `docs/PR_BOT_EVAL_SNIPPET.md` | Baixo | Evita mexer no template global sempre |

**Decisão recomendada:** priorizar documentação canônica e snippets auxiliares para reduzir conflito sem perder governança.


## Revalidação pós-mudanças (2026-03-13) ✅

| Pergunta | Resposta |
|---|---|
| Ainda é necessário implementar esses artefatos? | Parcialmente: a base já foi implementada e deve ser mantida. |
| O que ainda precisa? | Governança contínua (auditoria mensal), ajuste de budgets com baseline real e limpeza de itens obsoletos. |
| Recomendação | Não remover; consolidar e reduzir duplicações gradualmente. |
