...
class Engine {
    ...
    public static function award_points($user_id, $amount, $action, $ref_id = 0, $ref_type = '', $description = '') {
        ...
        // 2. Update User Balance (Meta) and GamiPress
        if (\function_exists('gamipress_award_points_to_user')) {
             $slug = \function_exists('djz_get_gamipress_points_type_slug') 
                 ? \djz_get_gamipress_points_type_slug() 
                 : 'points';
             
             \gamipress_award_points_to_user($user_id, (int)$amount, $slug, [
                 'reason' => $description ?: $action,
                 'reference_id' => $ref_id,
                 'reference_type' => $ref_type
             ]);
             
             $new_balance = \gamipress_get_user_points($user_id, $slug);
        } else {
