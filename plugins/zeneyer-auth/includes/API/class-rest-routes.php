...
class Rest_Routes
{
    ...
    public static function get_orders($request)
    {
        ...
        if (!empty($order_ids)) {
            ...
            // ⚡ Bolt: Prevent N+1 queries by bulk fetching order items and their meta in a single SQL query
            $sql = "
                SELECT
                    i.order_id,
                ...
                LEFT JOIN {$wpdb->prefix}woocommerce_order_itemmeta im
                    ON i.order_item_id = im.order_item_id AND im.meta_key IN ('_qty', '_line_total')
                WHERE i.order_id IN ($in_clause)
