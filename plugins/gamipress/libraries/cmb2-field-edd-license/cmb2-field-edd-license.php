                ...
                foreach( $cmb->meta_box['fields'] as $field ) {
                    ...
                    if( $field['type'] === 'edd_license' ) {
                        ...
                        if( $cmb->is_options_page_mb() ) {
                            ...
                            // On delete actions, $option_key is an array of deleted items
                            if( is_array( $option_key ) ) {
                                foreach ( $option_key as $key ) {
                                    $current_option_key = $key;

                                    if( ! $current_option_key && isset( $cmb->meta_box['show_on'] ) && isset( $cmb->meta_box['show_on']['value'] ) ) {
                                        if( is_array( $cmb->meta_box['show_on']['value'] ) ) {
                                            $current_option_key = $cmb->meta_box['show_on']['value'][0];
                                        } else {
                                            $current_option_key = $cmb->meta_box['show_on']['value'];
                                        }
                                    }
                                     ...
                                     * @since 1.0.0
                                     *
                                     * @param int|string $current_option_key
                                     * @param CMB2 $cmb
                                     *
                                     * @return int|string
                                     */
                                    $current_option_key = apply_filters( 'cmb2_edd_license_option_key', $current_option_key, $cmb );

                                    $default = isset( $field['default'] ) ? $field['default'] : '';

                                    $args['value'] = cmb2_get_option( $current_option_key, $field['id'], $default );
