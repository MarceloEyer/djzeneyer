...
function djz_get_multilang_menu_handler($request) {
    ...
    // 1. Tenta encontrar por slugs comuns
    $menu_id = $locations['primary'] ?? $locations['primary_menu'] ?? $locations['header_menu'] ?? null;
    
    // 2. Fallback: Pega o primeiro menu existente
    if (!$menu_id) {
    ...
    foreach ($items as $item) {
        ...
        $url = $item->url;
        // Corrige links internos para SPA
