 ...
 * v4.1.0 - Hardened and Optimized Version
 */
...
add_action('wp_enqueue_scripts', function () {
    ...
    wp_enqueue_script('djzeneyer-react', get_template_directory_uri() . '/dist/assets/index.js', array(), '4.1.0', true);
    wp_enqueue_style('djzeneyer-react-styles', get_template_directory_uri() . '/dist/assets/index.css');
    wp_localize_script('djzeneyer-react', 'wpData', array('siteUrl' => get_site_url(), 'restUrl' => get_rest_url(), 'nonce' => wp_create_nonce('wp_rest')));
});
...
add_action('after_setup_theme', function () {
    ...
    register_nav_menus(array('primary_menu' => __('Menu Principal', 'djzeneyer')));
});
...
add_action('init', function () {
    ...
    header("Access-Control-Allow-Credentials: true"); // ASPAS CORRIGIDAS AQUI
...
add_action('rest_api_init', function () {
    ...
    register_rest_route($namespace, '/menu', ['methods' => 'GET', 'callback' => 'djzeneyer_get_menu_items_callback', 'permission_callback' => '__return_true', 'args' => ['lang' => ['validate_callback' => function($param) { return is_string($param); }]]]);
    register_rest_route($namespace, '/checkout', ['methods'  => 'POST', 'callback' => 'djzeneyer_process_checkout_callback', 'permission_callback' => function($request) { return wp_verify_nonce($request->get_header('X-WP-Nonce'), 'wp_rest'); }]);
...
function djzeneyer_get_menu_items_callback(WP_REST_Request $request) {
    ...
    $menu_items = get_transient($cache_key); // Tenta buscar do cache

    if (false === $menu_items) { // Se não estiver no cache, busca no banco
        ...
        // Salva o resultado no cache por 1 hora
...
function djzeneyer_process_checkout_callback(WP_REST_Request $request) {
    // ... (código do checkout com validação de email) ...
    $params = $request->get_json_params();
    if (empty($params['email']) || !is_email($params['email'])) {
        return new WP_Error('invalid_email', 'O endereço de email fornecido é inválido.', ['status' => 400]);
    }
    // ... (resto da função de checkout) ...
    try {
        // ...
    } catch (Exception $e) {
