...
## 2026-05-12 - Stable empty array fallback in all files

**Learning:** Conditionally mapping undefined data to inline empty arrays (`|| []`) inside functional React components creates a brand new array reference on every render. If this inline array is passed into the dependency array of a `useMemo` hook or `.map()`, it will defeat memoization and cause the hook/UI to needlessly re-evaluate and iterate on every React reconciliation cycle.
**Action:** Always declare a properly typed constant array (e.g., `const EMPTY_PRODUCT_ARRAY: WCProduct[] = [];`) at the module scope (outside the component) and use that constant as the fallback.
