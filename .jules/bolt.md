...
## 2026-10-25 - Fix Safari Date Parsing Bug\n**Learning:** When parsing WordPress date strings (e.g., `YYYY-MM-DD HH:MM:SS`) into JavaScript `Date` objects in the frontend, safely replace the space with a `'T'` using a regex (e.g., `dateString.replace(/^(\d{4}-\d{2}-\d{2})\s(\d{2}:\d{2}:\d{2})/, '$1T$2')`) to prevent `Invalid Date` errors in Safari without breaking other valid date formats.\n**Action:** Always use the regex-based replacement when converting potential WordPress date strings to ensure cross-browser compatibility and prevent unintended side effects on standard date formats.
