...
## 2024-11-20 - WooCommerce Headless Filtering Architecture\n**Learning:** In headless WooCommerce architectures, never fetch the entire product catalog to filter it client-side. Always use native REST API parameters to filter, sort, and paginate at the database level.\n**Action:** Always: backend filters / frontend renders. Never: backend sends all / frontend filters. Use query parameters like `?featured=true&per_page=1`, `?orderby=date&order=desc&per_page=10`, and `?orderby=popularity&per_page=10` instead of JavaScript array operations.
