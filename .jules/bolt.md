...
## 2024-05-18 - Prevent React Reconciliation Reallocation for Arrays
**Learning:** Defining inline arrays (like `[...]`) directly inside a React component's render body forces JavaScript to allocate a brand new array object in memory on every render cycle. When this inline array is chained into a `.map()`, it unnecessarily triggers garbage collection overhead and can degrade performance on frequently updated views.
**Action:** Always extract static inline arrays to module-level constants outside the component. For arrays containing localized strings (like `t('...')`), extract them into a `useMemo` hook using `t` as a dependency to retain translation reactivity while preventing render loop reallocation.
