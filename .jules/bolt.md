...
## 2026-04-23 - Extract Date Parsing from Render Loops\n**Learning:** Instantiating `new Date()` or calling `Date.now()` inside React render bodies, `useMemo` hooks, or array `.map()` iterations causes unnecessary object allocations and violates React hook purity (triggering ESLint warnings or unexpected re-evaluations).\n**Action:** Always extract static dates into global constants (e.g., `CURRENT_YEAR` in `artistData.ts`), lazily initialize state `const [currentDate] = useState(() => Date.now())`, or use lightweight operations like string slicing (`String(date).substring(0, 4)`) inside loops to prevent reallocation overhead and maintain pure render cycles.
